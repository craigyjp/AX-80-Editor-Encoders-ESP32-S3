/*
  AX80 Editor Encoders - Firmware Rev 1.4
...
//SPIClass spiSD = SPIClass(HSPI); // Use HSPI
...
// Your callback function declarations
...
void myPitchBend(byte channel, int value);
void handleSysexByte(byte *data, uint16_t length, bool last);

void setup() {

  Serial.begin(115200);

  SPI.begin(12, 13, 11);  // SCLK, MISO, MOSI — use your wiring
  setupDisplay();
  ...
  // --------------------------------------------------
  // Load patches, or create the first one if empty

  cardStatus = LittleFS.begin();
  if (cardStatus) {
    Serial.println("LittleFS is connected");
    //Get patch numbers and names from SD card
    loadPatches();
    if (patches.size() == 0) {
      //save an initialised patch to LittleFS
      savePatch("1", INITPATCH);
      loadPatches();
    }
  } else {
    Serial.println("LittleFS is not connected or unusable");
    reinitialiseToPanel();
    showPatchPage("No LittleFS", "conn'd / usable");
  }

  // --------------------------------------------------
  // Recall first patch automatically
  // --------------------------------------------------
  ...
  //MIDI 5 Pin DIN
  Serial1.begin(31250, SERIAL_8N1, 18, 17);  // RX, TX
  MIDI.begin();
  ...
  // Set USB properties BEFORE beginning USB
  USB.manufacturerName("Craig Barnes");
  USB.productName("AX-80 Editor");
  USB.serialNumber("123457");

  USB.begin();
  delay(100);  // Small delay after USB.begin()
  myMIDI.begin();

  Serial.println("myMIDI In USB Listening");


  Serial2.begin(31250, SERIAL_8N1, 5, 6);  // RX, TX
  MIDI5.begin();
...
void myControlChange(byte channel, byte control, int value) {
  switch (control) {
    ...
    case CCosc1_octave:
      if (value <= 2) {  // only act if current octave <= 2
        osc1_octave = value;
        updateosc1_octave();
      }
      break;
...
void recallPatch(int patchNo) {
  ...
  filename += String(patchNo);
  //filename += ".txt";

  File patchFile = LittleFS.open(filename, "r");
  if (!patchFile) {
...
void handleMIDIPacket(uint8_t *packet) {
  uint8_t code = packet[0] & 0x0F;
  uint8_t b1 = packet[1];
  uint8_t b2 = packet[2];
  uint8_t b3 = packet[3];

  byte channel = (b1 & 0x0F) + USB_CHANNEL_OFFSET;
  byte statusType = b1 & 0xF0;

  switch (code) {
    case 0x08:  // Note Off
      myNoteOff(channel, b2, b3);
      break;
    case 0x09:  // Note On
      if (b3 == 0) {
        myNoteOff(channel, b2, b3);
      } else {
        myNoteOn(channel, b2, b3);
      }
      break;
    case 0x0B:  // Control Change
      myConvertControlChange(channel, b2, b3);
      break;
    case 0x0C:  // Program Change
      myProgramChange(channel, b2);
      break;
    case 0x0D:  // After Touch
      myAfterTouch(channel, b2);
      break;
    case 0x0E:  // Pitch Bend
      {
        int bend_value = ((b3 & 0x7F) << 7) | (b2 & 0x7F);
        myPitchBend(channel, bend_value);
      }
      break;
  }
}
